...
run_python() {
  ...
  model_version=$2
  method=$3
  LOGFILE="explain_batch_all_log_${dataset}_model_${model_version}.txt"
  ...
  python explain.py --dataset "$dataset" --model_version "$model_version" --num_pairs 50 --explain_method "$method" >> "$LOGFILE" 2>&1
  if [ $? -ne 0 ]; then
